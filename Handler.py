...
from messages import (
    ...
    replace_response_classes_translation,
    replace_response_classes_translation,
    AGREEMENT_QUESTION,
    THANKS_MESSAGE,
...
class Handler:
    ...
    async def receive_photo(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        print(
            {
                "telegram_img_id": file_id,
                "telegram_conversation_id": update.message.chat_id,
            }
        )
        ...
        if (analysis_result_dict["type"]["name"]) == FruitType.BANANA.value:
            FEEDBACK_TEMP["model_stage"] = analysis_result_dict["maturation_stage"][
                "name"
            ]
            context.chat_data[KEY] = FEEDBACK_TEMP

        message = replace_response_classes_translation(
            self.sender.define_predict_response_obj(analysis_result_dict)
        )
    ...
    async def callback_handler(self, update: Update, context: CallbackContext):
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=f"Sua resposta foi: {replace_response_classes_translation(query.data)}",
        )
        ...
        if query.data in [member.value for member in Confirmation]:
            ...
            await self.validate_confirm_response(update, context, query.data)
            ...
            print(
                "[FRUIT-LENS][FRUIT-MATURATION-STAGE] - Seleção de estagio de maturacao de fruta do usuario"
            )
    ...
    async def validate_confirm_response(
        ...
        if response == Confirmation.YES.value:
            ...
            payload = {
                "user_class_fruit_type": chat_data["model_type"],
                "user_class_maturation_stage": chat_data["model_stage"],
                "user_approval": True,
            }
    ...
    async def select_fruit_options(
        ...
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=FRUIT_CLASSIFICATION_QUESTION,
            reply_markup=keyboard_reply,
        )
    ...
    async def select_fruit_stage(
        ...
        keyboard_reply = InlineKeyboardMarkup(
            inline_keyboard=[
                ...
                [
                    InlineKeyboardButton(
                        "Podre 🤮", callback_data=FruitStage.ROTTEN.value
                    )
                ],
            ]
        )
        await context.bot.send_message(
            chat_id=update.effective_chat.id,
            text=STAGE_CLASSIFICATION_QUESTION,
            reply_markup=keyboard_reply,
        )
    ...
    async def validate_fruit_type(
        ...
        print(response)
        if (
            response == FruitType.BANANA.value
            and response == fruit_selection["model_type"]
        ):
            ...
            chat_data = context.chat_data.get(KEY, "Not found")
            payload = {
                "user_class_fruit_type": response,
    ...
    async def validate_fruit_stage(
        ...
        payload = {
            ...
            "user_class_maturation_stage": response,
            "user_approval": False,
        }
        response = await self.sender.send_request(
            BASE_URL + FEEDBACK_URI + f'/{chat_data["img_id"]}',
